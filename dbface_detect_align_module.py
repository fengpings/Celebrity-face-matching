...
class dbface_detect():
    ...
    def detect(self, srcimg, threshold=0.3, nms_iou=0.3):
        ...
        with torch.no_grad():
            ...
            k = hm_pool.shape[2] * hm_pool.shape[3]
            if k > 1000:
                scores, indices = ((hm == hm_pool).float() * hm).view(1, -1).cpu().topk(1000)
            else:
                scores, indices = ((hm == hm_pool).float() * hm).view(1, -1).cpu().topk(k)
        ...
        for i, obj in enumerate(objs):
            ...
            face_roi = srcimg[(0 if box[1] < 0 else box[1]):box[3], (0 if box[0] < 0 else box[0]):box[2]]
            # import imutils
            ...
            if self.align:
                ...
                #     landmark[i][0] = landmark[i][0] - box[0]
                #     landmark[i][1] = landmark[i][1] - box[1]
                #     box = [0,0,face_roi.shape[1],face_roi.shape[0]]
#                 h, w = srcimg.shape[:2]
...
#                     h = 224
#                     w = round(h * ratio)
#                 pad_bottom = abs(224 - h)
#                 pad_right = abs(224 - w)

#                 scaled_img = cv2.resize(srcimg, (w, h), interpolation=interp)
#                 image = cv2.copyMakeBorder(
#                     scaled_img,0,pad_bottom,0,pad_right,borderType=cv2.BORDER_CONSTANT,value=[0,0,0])
#                 landmark = np.array(landmark).reshape((5,2))
#                 for i in range(len(landmark)):
#                     landmark[i][0] = landmark[i][0]
#                     landmark[i][1] = landmark[i][1]
#                     box = [0,0,face_roi.shape[1],face_roi.shape[0]]
    ...
    def get_face(self, srcimg, threshold=0.3, nms_iou=0.3):
        ...
        for i, obj in enumerate(objs):
            box, score, landmark = list(map(int, obj.box)), obj.score, obj.landmark
            landmark = [int(x) for t in obj.landmark for x in t]
            boxs.append(box+[score]+landmark)

            face_roi = srcimg[int(box[1]):int(box[3]), int(box[0]):int(box[2])]
            if self.align:
                face_roi = align_process(srcimg, np.array(box), np.array(landmark).reshape(-1, 2), (224, 224))
            face_rois.append(face_roi)
